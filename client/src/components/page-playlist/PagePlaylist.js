...
import { fetchLoadSong } from "../../actions";
...
const PagePlaylist = () => {
  ...
  function getDuration(url, next) {
    ...
    _player.addEventListener("durationchange", (e) => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchLoadSong(audioSrc, audioRef, setDuration))

    setTrackProgress(audioRef.current.currentTime);

    if (isReady.current && typeof (duration) === "number") {
      audioRef.current.play();
      setIsPlaying(true);
      startTimer();
    } else {
        ...
        <div className="track-info">
          ...
          <div className="audio-time">
            ...
            <span className="audio-duration">
              {(duration && !isNaN(duration)) && calculateDurationTime(duration)}
            </span>
          </div>
        </div>
      </div>
