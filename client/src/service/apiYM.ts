...
(async () => {
    try {
        ...
        console.log({
            searchResult, gorillaz, gorillazMostPopularTrack
        });
        ...
        const getTracksSupplementResult = await api.getTrackSupplement(gorillazMostPopularTrackId);
        console.log({ getTracksSupplementResult });

        const getTrackDownloadInfoResult = await api.getTrackDownloadInfo(gorillazMostPopularTrackId);
        ...
        const getTracksDirectLinkResult = await api.getTrackDirectLink(hqMp3Track.downloadInfoUrl);
